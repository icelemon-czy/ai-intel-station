...
class DiscoverCliTests(unittest.TestCase):
    ...
    def test_discover_dry_run_with_real_example_succeeds(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            buf = io.StringIO()
            original_default = command_module.DEFAULT_CONFIG_PATH
            command_module.DEFAULT_CONFIG_PATH = tmp_path / "missing-default.yaml"
            try:
                with redirect_stdout(buf):
                    exit_code = run_discover(
                        command_module.DEFAULT_CONFIG_PATH,
                        only=None,
                        dry_run=True,
                        output_root=tmp_path / "output",
                    )
            finally:
                command_module.DEFAULT_CONFIG_PATH = original_default
