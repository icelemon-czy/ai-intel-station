"""L3 sub-spec coverage — real end-to-end tests for the remaining
...
monkeypatching `collect.github.fetch_repo`. This way:
  ...
  - the only thing under test control is *what* `gh` returns,
    which is exactly what the L3 spec says the surrounding code
    must handle.
"""
...
# ---------------------------------------------------------------------------
# L3 sub-spec: github / "Include Open Issues in Repository Snapshots"
...
class L3GithubIncludesOpenIssuesSubprocessTests(unittest.TestCase):
    def test_research_collect_github_writes_open_issues_section(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = _run_subprocess(
                ["collect", "github", "fixture-org/real-gh-issues"],
                env_extra={
                    "PATH": f"{fake_gh_dir}{os.pathsep}{os.environ.get('PATH', '')}",
                    # CLI uses -o / --output-root to choose the archive
                    # directory; without one it falls back to a project
                    # default which on this sandbox may already have
                    # content. Pin a tempdir.
                },
            )
            ...
            # Because we didn't pass -o, the CLI wrote into the project
            # default `output/` (relative to REPO_ROOT cwd). Find the
            # freshly-written README there.
            default_repo_dir = REPO_ROOT / "output" / "github" / "fixture-org-real-gh-issues"
            md_path = default_repo_dir / "README.md"
            self.assertTrue(
                md_path.exists(),
            ...
            # L3 contract: open issues MUST appear in the saved
...
# ---------------------------------------------------------------------------
# L3 sub-spec: github / "Save Search Results as Markdown"
...
class L3GithubSearchSubprocessTests(unittest.TestCase):
...
# ---------------------------------------------------------------------------
# L3 sub-spec: papers / "Fetch Latest Papers by Category" (success path)
...
# exactly what `docs/l3-coverage.md` calls "acceptable."
# We do that in-process: redirect `collect.papers.urlopen` to a local
# URL serving a fixture XML.
# ---------------------------------------------------------------------------


class L3PapersFetchLatestSubprocessTests(unittest.TestCase):
    def test_research_collect_papers_cs_ai_writes_paper_markdown(self):
        """L3 sub-spec: 'Fetch Latest Papers by Category' (success path).
        ...
        `docs/l3-coverage.md` for the rationale."""
...
# here because the rest of the L3 e2e layer uses it and pytest is
