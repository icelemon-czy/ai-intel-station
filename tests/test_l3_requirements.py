...
# briefing/backfill/web/discover/schedule/init-config), not a source-
# specific wrapper.
# ---------------------------------------------------------------------------


class L3RunnableDocumentedEntrypointsTests(unittest.TestCase):
    def test_research_help_lists_all_documented_subcommands(self):
        result = _run_cli("--help")
        self.assertEqual(result.returncode, 0, msg=result.stderr)
        for cmd in ("collect", "query", "briefing", "backfill",
    ...
    def test_discover_help_documents_first_time_steps(self):
        # L3: "command documented in `.ai` or workspace README files".
...
# moved to `tests/test_l3_http_e2e.py::L3ExplicitFailureHttpBoundaryTests`,
...
# `collect.github.run_gh` is the kind of mock the L3 spec explicitly
# rules out.
# ---------------------------------------------------------------------------
...
class L3OptionalLiveVerificationTests(unittest.TestCase):
    def test_wechat_live_test_skips_when_urls_unset(self):
        ...
        # The run must complete cleanly. The L3 contract is "skip cleanly
...
class L3SourceSegregatedQueryTests(unittest.TestCase):
