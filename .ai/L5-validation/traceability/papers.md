...
| Save One Markdown File per Paper | Save fetched papers | `paper_to_markdown()` + `save_papers()` | `tests/test_research_item.py::test_save_papers_writes_markdown_and_research_item_sidecar` | verified | tmp_path 文件系统测试已断言 Markdown 文件、标题和作者内容 |
...
| Persist paper-side ResearchItem sidecar | Paper markdown generated | `save_papers()` + `build_paper_item()` | `tests/test_research_item.py::test_save_papers_writes_markdown_and_research_item_sidecar` | verified | tmp_path 文件系统测试已断言 `<stem>.research-item.json` 与 paper metadata |
