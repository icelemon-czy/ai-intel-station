...
- **做什么**: 通过统一的 workspace 入口完成资料收集、sidecar 回填、本地查询和简报生成
- **技术栈**: Python CLI + uv + pytest(根级 ResearchItem + wechat) + gh CLI + arXiv API + Camoufox / BeautifulSoup / markdownify
- **当前业务结构**: `research/` → `collect/` → `library/` → `briefing/` → `publish/`
