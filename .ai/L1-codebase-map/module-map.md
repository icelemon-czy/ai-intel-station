...
| 模块 | 路径 | 对外暴露 | 状态 |
|------|------|----------|------|
| Research Operations | `research/` | `main()`、`collect/query/briefing/backfill` 子命令 | 🟢 active |
| Collect | `collect/` | `github.main()`、`papers.main()`、`wechat.main()` 及各来源收集函数 | 🟢 active |
| Library | `library/` | `ResearchItem`、backfill、sidecar 扫描、查询入口 | 🟢 active |
| Briefing | `briefing/` | digest / reading-list 生成与 CLI | 🟢 active |
| Publish | `publish/` | Obsidian 友好路径和写文件辅助 | 🟢 active |
| Archive Output | `output/` | 原始抓取归档 + `output/briefing/` 派生阅读产物 | 🟢 stable |
| AI Context | `.ai/` + `AGENTS.md` + `CLAUDE.md` | 导航、规则、spec、workflow 说明 | 🟡 active |
...
| `publish/` → `output/github|papers|wechat` | ❌ | — | 派生产物必须与原始归档隔离 |
...
| `collect/wechat.py` 的输出目录或 Markdown 模板 | `tests/test_wechat_collect.py`、`.ai` 文档 | 真实实现已直接暴露给统一入口 |
| `collect/github.py` 的 repo / search 输出命名 | `output/github/` 样例、`.ai` 文档 | 目录命名和搜索 sidecar 约定会影响查询层 |
