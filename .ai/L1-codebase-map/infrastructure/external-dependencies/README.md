# 外部依赖

## 依赖清单

| 依赖 | 被谁使用 | 作用 | 典型失败信号 |
|------|----------|------|--------------|
| Camoufox | wechat | 绕过 WeChat 抓取限制并获取页面 HTML | 浏览器启动失败、抓取不到正文 |
| httpx | wechat | 下载图片到本地 | 图片缺失、链接未重写 |
| `gh` CLI | github | 查询仓库、议题、搜索结果 | `run_gh()` 抛 `RuntimeError` |
| arXiv API | papers | 拉取 Atom feed | 某类别打印 `Failed to fetch` |
| 网络访问 | 全部抓取工具 | 访问远程源 | 命令超时、空输出、解析失败 |

## 先验检查

```bash
gh auth status
uv run research collect papers --list
uv run --with pytest python -m pytest tests/test_wechat_collect.py
```

## 故障分类

1. **认证型**: GitHub 工具最常见，先看 `gh` 是否登录。
2. **浏览器运行时型**: 只影响 wechat，通常和 Camoufox 或页面反爬有关。
3. **公开 API / 网络型**: papers 和 wechat 都会受影响，但 papers 通常表现为单类别 warning。
4. **格式漂移型**: 外部页面结构变了，现有解析逻辑和测试不再匹配。

## 修改原则

- 不要把外部依赖问题伪装成输出层问题；先定位在哪个边界失败
- 依赖检查命令写进文档后，代码改动时要保持它们仍然可用
- 如果某项依赖变成新的硬前置条件，要同步更新 feature README、L2 规则和 L3 spec