...
3. 如果子命令变化会影响输出或行为说明，同步更新 README、AGENTS、CLAUDE、.ai。
...
**参考已有实现**: `collect/github.py`、`collect/papers.py`、`collect/wechat.py`

```text
步骤：
1. 先确认真实实现是不是在 `collect/<source>.py`，不要回到历史来源目录找入口。
2. 把“拉取数据 → 转 Markdown → 落盘 → sidecar”保持成单条清晰管线。
3. 如果改动影响输出结构或 Markdown 字段，同步更新 output 说明和 .ai 文档。
4. WeChat 的纯转换变化优先补 `tests/test_wechat_collect.py`。
5. GitHub / papers 变化至少保留一条最小 smoke 或测试计划。
...
**参考已有实现**: `library/query.py`、`briefing/reports.py`、`publish/obsidian.py`

```text
步骤：
1. 查询层只能消费本地 sidecar，不要回 collect 层重新抓远端数据。
2. briefing 只能写 `output/briefing/`，不要覆盖原始归档。
3. 部分成功必须继续保留，并显式写出缺失来源。
...
| 当你改了这个文件 | 影响范围 | 必须做的事 |
|-----------------|----------|-----------|
| `research/cli.py` | 统一 operator surface、命令语义、根级运行方式 | 跑 `tests/test_restructure_research_architecture.py`，同步 README / AGENTS / CLAUDE / `.ai` |
...
| briefing 缺来源或排序异常 | `build_*_markdown()` → `briefing_output_path()` | 先区分是查询结果缺失，还是报告层展示问题 |
| 生成内容和文档不一致 | README / SKILL / `.ai` 文档 | 这类问题常常不是代码错，而是命令或路径说明未同步 |
