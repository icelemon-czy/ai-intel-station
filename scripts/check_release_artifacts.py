...
def _static_entry_name(names: list[str], suffix: str) -> str:
    matches = [
        name
        for name in names
        if name.strip("/").endswith(f"workspace_web/static/{suffix}")
    ]
    if len(matches) != 1:
...
def _assert_web_assets(
    ...
    *,
    ...
    normalized = [name.strip("/") for name in names]
    static_names = [
        name[name.index("workspace_web/static/") :]
        for name in normalized
        if "workspace_web/static/" in name
    ]
    required = "workspace_web/static/index.html"
    ...
    for reference in parser.references:
        ...
        if relative.startswith("assets/"):
            local_assets.append(f"workspace_web/static/{relative}")
