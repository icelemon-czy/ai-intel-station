...
const STATIC_DIR = resolve(__dirname, "../../src/ai_intel_station/adapters/web/static");
const ASSET_DIR = resolve(STATIC_DIR, "assets");

function loadBundle() {
    const files = readdirSync(ASSET_DIR).filter((name) => name.endsWith(".js"));
    assert.ok(files.length > 0, "no JS bundle found — has `npm --prefix frontend run build` been run?");
