...
from ai_intel_station.library.service import (
    backfill_library,
...
def _resolve_discovery_log_dir(config_path: Path) -> Path:
    ...
    if log_dir is None:
        from ai_intel_station.discovery.config import DEFAULT_LOG_DIR
        log_dir = DEFAULT_LOG_DIR
    return log_dir


def run_discover_status(config_path: Path, *, last: int = 1) -> int:
    from ai_intel_station.discovery import latest_log_path, read_log_summary, recent_log_paths
