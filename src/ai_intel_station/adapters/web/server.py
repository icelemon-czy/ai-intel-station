    ...
    class WorkspaceHandler(BaseHTTPRequestHandler):
        ...
        def _serve_static(self, request_path: str) -> None:
            ...
            if not index_path.exists():
                _text_response(
                    self,
                    "Frontend assets are missing. Run `npm --prefix frontend install && npm --prefix frontend run build` first.",
...
def serve_workspace(output_root: Path, host: str = "127.0.0.1", port: int = 4173) -> None:
    ...
    # from `frontend/` would resolve `output` as `frontend/output`).
    #
    # We anchor relative paths to the project root (the repository root, four levels above
    # `adapters/web/server.py`) so the same script works from
