...
- 根目录 `pyproject.toml` 维护统一依赖、optional extras 和 `research` console script
- default dependency 只承担 core command surface；当前为 PyYAML
- WeChat 的 Camoufox / BeautifulSoup / markdownify / httpx 位于 `wechat` extra，
  使用前运行 `uv sync --extra wechat`
- pytest 位于 `dev` extra，不进入 normal operator bootstrap
...
3. 新依赖先判断是 core、source-specific 还是 test-only；分别放 base、对应 optional extra
   或 `dev` extra，不让关闭状态的 source 扩大 default bootstrap。
4. 如果改子命令语义，要同步 README / SKILL / `.compass/context`。
