...
| Daily Signal Briefing | Default 5 News/up to 2 optional WeChat + 1 GitHub + 1 arXiv composition | `briefing/signals.py::select_daily_briefing`, `research/discovery/runner.py::generate_briefing` | `tests/test_realtime_signals.py` cap/replacement/shortfall selector + real production generate assertions | ✅ verified |
...
| Honest Empty Signal Result | quota bounds/type/relation/source validation and positive-minimum migration | `research/discovery/config.py` | `tests/test_realtime_signals.py` explicit-empty, integer, conflict, optional-source, explicit-zero and positive-minimum assertions | ✅ verified |
