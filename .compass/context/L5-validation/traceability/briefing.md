...
| Distinct Target and Signal Attribution Links | HN title target + discussion attribution, historical fallback, X/WeChat canonical and dry-run source maxima | `briefing/signals.py::_entry_markdown`, `research/discovery/runner.py::generate_briefing` | `tests/test_realtime_signals.py` source-native link + dry-run source-maxima assertions | ✅ verified |
| Daily Signal Briefing | Default 3 Hacker News / optional max 2 WeChat / 1 GitHub / 1 arXiv grouped composition | `briefing/signals.py::select_daily_briefing`, `research/discovery/runner.py::generate_briefing` | `tests/test_realtime_signals.py` source-grouped selector + production generate assertions | ✅ verified |
...
| Daily Signal Briefing | Required source shortfall + dedicated corroboration confidence | `briefing/signals.py` | `tests/test_realtime_signals.py` no-failure shortfall, low/medium/high and grouped renderer assertions | ✅ verified |
...
| Honest Empty Signal Result | quota bounds/type/relation/source validation, news_items migration and ignored destination field | `research/discovery/config.py`, `briefing/signals.py` | `tests/test_realtime_signals.py` production YAML, integer, conflict and migration assertions | ✅ verified |
