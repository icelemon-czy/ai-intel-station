...
| Explicit Signal and Evidence Roles | Dedicated-only evidence + papers>github>hackernews>wechat>x ownership | `briefing/signals.py::select_daily_briefing` | `tests/test_realtime_signals.py` replacement/no-replacement/corroboration assertions | ✅ verified |
| Explicit Signal and Evidence Roles | Per-source HN/WeChat quotas, GitHub-target HN stays in Hacker News, optional WeChat cap and legacy minimum | `briefing/signals.py::DailyBriefingSelection`, `select_daily_briefing` | `tests/test_realtime_signals.py` source-grouped selector, github-target HN, duplicate/mixed-group and positive minimum assertions | ✅ verified |
...
Destination-host News cap 已从主 Spec 删除；HN GitHub-target 归属由 collect `source` 决定。
