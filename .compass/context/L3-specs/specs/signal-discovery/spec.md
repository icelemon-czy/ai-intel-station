...
- **WHEN**至少 2 条 verified fresh WeChat item 与足够其他 ranked News signal 存在
- **THEN**News lane 先包含至少 2 条 WeChat item，再填充 remaining position
- **AND**其余位置按 normal deterministic ranking 从 HN、X 与 unused WeChat 选择

#### Scenario: WeChat minimum cannot be met

- **WHEN**少于 2 条 verified fresh WeChat item，但其他 News signal 可用
- **THEN**其他 signal MAY 填满 5 条 News
- **AND**quota coverage 仍报告 missing WeChat minimum，briefing 保持 `partial`

`actual_wechat` SHALL 按 deduplicated rendered News entry 计数：entry 只要包含至少一个 WeChat
contribution 就计 1。多个 raw WeChat 合并后只计 1；mixed WeChat/HN group 也只计 1。只作为
dedicated GitHub/Paper corroboration 的 WeChat 不计入 News WeChat minimum。

#### Scenario: Duplicate WeChat signals count after News dedupe

- **WHEN**两个 raw WeChat candidate normalize 为一个 News entry，另一 separate WeChat/HN group normalize 为第二个
- **THEN**两个 rendered News entry 计为 `actual_wechat=2`
- **AND**raw candidate count 不得虚假满足 WeChat minimum
